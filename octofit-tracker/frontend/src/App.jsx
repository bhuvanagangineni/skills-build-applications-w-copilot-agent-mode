...
function Overview() {
  ...
  useEffect(() => { Promise.all([fetch(apiUrl('/api/activities/')).then((r) => r.json()), fetch(apiUrl('/api/leaderboard/')).then((r) => r.json())]).then(([a, l]) => { setActivities(Array.isArray(a) ? a : a.results || []); setLeaderboard(Array.isArray(l) ? l : l.results || []); }).catch(() => {}); }, []);
